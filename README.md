...
shutdown -r now
```
After restart log back in and then check mongodb service ok
```
...
```
Start the app using pm2 command
```
...
WantedBy=multi-user.target
```
Check consumer service running status
```
...
```
Check consumer parts service running status
```
